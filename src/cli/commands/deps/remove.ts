...
export interface DepsRemoveResult {
  success: boolean;
  error?: string;
}
...
export async function runDepsRemove(options: DepsRemoveOptions): Promise<DepsRemoveResult> {
  ...
  try {
    ...
    removeFudaDependency(db, fuda.id, dependsOn.id);
